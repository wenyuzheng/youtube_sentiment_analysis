...
class ModelTrainer:
  ...
  def load_model(self, model_name = 'xlm-roberta-base'):
  ...
  def train(self):
    ...
    for epoch in range(0, self.num_epochs):
      ...
      heading('Training Loss: {:}'.format(self.total_train_loss))
  ...
  def process_training_batch(self, batch, batch_id):
    ...
    self.total_train_loss = self.total_train_loss + loss.item()
  ...
  def save_model(self):
    ...
    self.model.save_pretrained('./sentiments/model-xlmr')
    self.tokenizer.save_pretrained('./sentiments/model-xlmr')
  ...
  def encode_train(self, text, label):
    ...
    return (padded_token_list, attention_mask, target)
